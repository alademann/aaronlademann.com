...
(function($) {
	$.reject = function(opts) {
		...
		var headerHTML = opts.header;
		var footerHTML = (opts.close ? '<p>' + opts.closeMessage + '</p>' : '');
		var bodyHTML = (opts.paragraph1 === '' ? '' : '<p>' + opts.paragraph1 + '</p>') +
		...
		for(var x in opts.display) {
			...
			bodyHTML += '<li id="jr_' + browser + '"><div class="jr_icon"></div>' +
		...
		// add content to html elem already in the html dom via footer.php
		var element = $("#jr_wrap"); // main wrapper elem
		...
		*/
		...
		element.find('.modal-body li').css({ // Browser list items (li)
			background: 'transparent url("' + opts.imagePath + 'background_browser.gif")' +
					'no-repeat scroll left top'
		});

		element.find('.modal-body li .jr_icon').each(function() {
		...
		element.find('.modal-body li a').click(function() {
		...
		//$('body').append(element);
		//$("#jr_modal").modal(); // use twitter bootstrap modal to pop open

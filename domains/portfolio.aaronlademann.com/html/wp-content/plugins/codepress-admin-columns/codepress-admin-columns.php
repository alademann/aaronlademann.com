...
{	
	...
	public function manage_posts_column_value($column_name, $post_id) 
	{
		...
		switch ($type) :			
			...
			// Likes
			case "_likes" :
				$result = $this->strip_trim(get_post_meta( $post_id, '_likes', true ));
				break;
	...
	private function get_uniform_format($columns) 
	{
		...
		foreach ( (array) $columns as $id => $label ) {
			...
			// likes exception
			//if ( strpos( $label, 'like-grey-heart.png') ) {
			//	$type_label 	= __('_likes', $this->textdomain);
			//	$hide_options 	= true;
			//}
	...
	private function get_custom_posts_columns($post_type) 
	{
		...
		// Like support
			$custom_columns['_likes'] = array(
				'label'			=> __('_likes', $this->textdomain),
				'options'		=> array(
					'type_label' 	=> __('_likes', $this->textdomain)
				)
			);
